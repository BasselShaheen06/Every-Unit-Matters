...
class InventoryDPSolver:
    ...
    def _dp(self, t, current_inv, p1, p2, p3):
        ...
        # Store in DP table for visualization
        state_key = (t, current_inv, p1, p2, p3)
        self.dp_table[state_key] = best_cost
        self.decision_table[state_key] = best_q
...
class InventoryGUI(tk.Tk):
    ...
    def run_solver(self):
        ...
        # Update DP visualization tables
        self.display_dp_tables()

    # ---------- DP TABLE DISPLAY ----------
    def display_dp_tables(self):
        if self.solver is None or not self.solver.dp_table:
            return
        
        # Get all unique inventory levels and periods from stored states
        periods = sorted(set(state[0] for state in self.solver.dp_table.keys()))
        inventories = sorted(set(state[1] for state in self.solver.dp_table.keys()))
        
        # Display DP cost table (simplified view - just t and inventory, averaging over pipeline)
        columns = ["Period"] + [f"Inv={i}" for i in inventories[:20]]  # Limit columns
        ...
        # Fill DP table (showing simplified state space)
        for t in periods:
            row = [f"t={t}"]
            for i in inventories[:20]:
                # Find states matching this t and inventory level
                matching_states = [v for k, v in self.solver.dp_table.items() 
                                 if k[0] == t and k[1] == i]
                if matching_states:
                    avg_cost = sum(matching_states) / len(matching_states)
        ...
        for t in periods:
            row = [f"t={t}"]
            for i in inventories[:20]:
                matching_decisions = [v for k, v in self.solver.decision_table.items() 
                                    if k[0] == t and k[1] == i]
                if matching_decisions:
                    avg_decision = int(sum(matching_decisions) / len(matching_decisions))
                    row.append(f"{avg_decision}")
                else:
                    row.append("-")
